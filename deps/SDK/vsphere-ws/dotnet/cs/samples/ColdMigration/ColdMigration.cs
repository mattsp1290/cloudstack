...
///</remarks>
    ...
    class ColdMigration
    {
        ...
        private Boolean registerVirtualMachine() {
           ...
           if(hostList.Count < 1) {
              Console.WriteLine("No host found in datacenter to"
                                      +" register the Virtual Machine");
              return registered;
           } else {
             Boolean hostFound = false; 
             for(int i=0; i<hostList.Count; i++) {
                ManagedObjectReference hostMor = (ManagedObjectReference)((object[])(hostList[i]))[0];
                ManagedObjectReference [] datastores = (ManagedObjectReference [])cb.getServiceUtil().GetDynamicProperty(hostMor,"datastore");

                long dirSize = getDirSize(getLocalPath());
                for(int j=0; j<datastores.Length; j++) {
                   DatastoreSummary datastoreSummary = (DatastoreSummary)
                            cb.getServiceUtil().GetDynamicProperty(datastores[j],"summary");

                   if(datastoreSummary.name.Equals(getDataStore())) {
             ...
             if(hostFound) {
                ...
                ManagedObjectReference hostFolderRef = (ManagedObjectReference)cb.getServiceUtil().GetDynamicProperty(dcmor, "hostFolder");
                // Get the compute resource from host folder.
                ManagedObjectReference[] crList = (ManagedObjectReference[])cb.getServiceUtil().GetDynamicProperty(hostFolderRef, "childEntity");
                ManagedObjectReference mor;
                ManagedObjectReference computeResourceRef = null;
                for (int i = 0; i < crList.Length; i++) {
                   mor = (ManagedObjectReference)(crList[i]);
                   if (mor.type.Equals("ClusterComputeResource")) {
                      computeResourceRef = mor;
                      break;
                   }
                }
                ...
                if (computeResourceRef != null) {
                    ...
                    ManagedObjectReference resourcePoolRootRef = (ManagedObjectReference)cb.getServiceUtil().GetMoRefProp(computeResourceRef, "resourcePool");
                    resourcePool = resourcePoolRootRef;
                } else {
                  resourcePool
               ...
               Console.WriteLine("No host in datacenter got the"
