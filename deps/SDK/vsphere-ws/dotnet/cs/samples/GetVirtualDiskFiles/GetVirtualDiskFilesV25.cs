...
namespace GetVirtualDiskFiles
{
    class GetVirtualDiskFilesV25
    {
        ...
        public static void Main(String[] args)
        {
            ...
            ecb = AppUtil.ExtendedAppUtil.initialize("GetVirtualDiskFilesV25"
        ...
        public void GetVirtualDiskFilesForHost()
        {
            try
            {
                ...
                ManagedObjectReference hmor = _service.FindByIp(ecb.getServiceConnectionV25().ServiceContent.searchIndex, null, ecb.get_option("hostname"), false);
                if (hmor == null)
                {
                    Console.WriteLine("Unable to find Host Syatem named : " + ecb.get_option("hostname") + " in Inventory");
                }
                else
                {
                    if (VersionUtil.isApiVersionSupported(supportedVersions, "2.5"))
                    {
                        ...
                        foreach (ManagedObjectReference dstore in dstoreArr)
                        {
                            ManagedObjectReference dsBrowser =
                                            ecb.getServiceUtilV25().GetMoRefProp(dstore, "browser");
                            ObjectContent[] objary = ecb.getServiceUtilV25().GetObjectProperties(_sic.propertyCollector, dstore, new String[] { "summary" });
                            ...
                            object[] result = ecb.getServiceUtilV25().WaitForValues(taskmor, new string[] { "info.state", "info.result" },
                            ...
                            {
                                ObjectContent[] objTaskInfo = ecb.getServiceUtilV25().GetObjectProperties(_sic.propertyCollector, taskmor, new String[] { "info" });
